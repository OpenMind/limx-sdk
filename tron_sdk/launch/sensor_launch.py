...
def generate_launch_description():
    ...
    serial_port = LaunchConfiguration(
        "serial_port",
        default=EnvironmentVariable("SERIAL_PORT", default_value="/dev/ttyUSB0"),
    )
    serial_baudrate = LaunchConfiguration(
        "serial_baudrate",
        default=EnvironmentVariable("SERIAL_BAUDRATE", default_value="115200"),
    )
    frame_id = LaunchConfiguration(
        "frame_id", default=EnvironmentVariable("FRAME_ID", default_value="laser")
    )
    inverted = LaunchConfiguration(
        "inverted", default=EnvironmentVariable("INVERTED", default_value="false")
    )
    angle_compensate = LaunchConfiguration(
        "angle_compensate",
        default=EnvironmentVariable("ANGLE_COMPENSATE", default_value="true"),
    )
